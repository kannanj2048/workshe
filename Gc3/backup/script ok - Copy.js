...
function getNowSGT() {
    // ✅ FIXED: Use toLocaleString to get actual SGT time
    const now = new Date();
...
function formatDateForInput(date) {
    // ✅ FIXED: Get SGT date components directly using SGT timezone
    ...
    if (form) {
        form.onsubmit = (e) => {
            ...
            if (applyToDate && schedule) {
                ...
                showNotification(`Shift updated to ${SHIFT_TYPES[newShift].name} for ${dateStr}!`);
            }
...
/*
// ============================================
// ✨ MONTHLY LEAVE MANAGEMENT SYSTEM v3.0
// ============================================
...
// ✅ THIS IS THE MISSING FUNCTION - Check if member is on leave for a specific date
...
function renderLeaveManagementContent() {
    ...
    html += `<div class="leave-section">
        <h4><i class="fas fa-shield-alt"></i> Security Analysts</h4>
...
// Toggle monthly leave for a member
...
function saveLeaveSettingsToFirebase() {
    if (typeof hasDatabase !== 'function' || !hasDatabase()) return;
...
function loadLeaveSettingsFromFirebase() {
    ...
    database.ref('leaveSettings').once('value')
        ...
        .catch(() => {}); // Silent fail - localStorage fallback works fine
}

console.log('%c✅ Monthly Leave Management System v3.0 Loaded', 'color: #10b981; font-weight: bold; font-size: 14px');
// ============================================
// END: MONTHLY LEAVE MANAGEMENT SYSTEM
// ============================================ */