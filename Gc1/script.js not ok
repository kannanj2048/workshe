...
function formatDateForInput(date) {
    // ✅ FIX: Ensure we're working with SGT timezone
    // Create a proper SGT date object
...
function removeMemberFromAvailability(name, role) {
    ...
    if (idx !== -1) {
        // ✅ FIX 1: Remove from team array
        teamData[category].splice(idx, 1);
        
        // ✅ FIX 2: Remove from responsibilities
        if (role === "SA" && teamData.responsibilities && teamData.responsibilities[name]) {
            delete teamData.responsibilities[name];
        }

        // ✅ FIX 3: Remove from memberAvailabilitySettings (CORRECT PATH)
        if (memberAvailabilitySettings[category] && memberAvailabilitySettings[category][name]) {
            delete memberAvailabilitySettings[category][name];
            saveMemberAvailabilitySettings();
            console.log(`✅ Removed ${name} from memberAvailabilitySettings.${category}`);
        }

        // ✅ FIX 4: Remove from availabilityOverrides
        const key = `${name}_${role}`;
        if (availabilityOverrides[key]) {
            delete availabilityOverrides[key];
            localStorage.setItem("availabilityOverrides", JSON.stringify(availabilityOverrides));
            ...
            console.log(`✅ Removed ${name} from availabilityOverrides`);
        }
        
        // ✅ FIX 5: Remove from monthlyLeaveSettings
        if (typeof monthlyLeaveSettings !== 'undefined') {
            const memberKey = `${name}|${role}`;
            let removedFromLeave = false;
            
            // Loop through all months and remove this member's leave
            for (const monthKey in monthlyLeaveSettings) {
                if (monthlyLeaveSettings[monthKey] && monthlyLeaveSettings[monthKey][memberKey]) {
                    delete monthlyLeaveSettings[monthKey][memberKey];
                    removedFromLeave = true;
                }
            }
            ...
            if (removedFromLeave) {
                ...
                console.log(`✅ Removed ${name} from leave calendar`);
            }
        }
        
        // ✅ FIX 6: Save to localStorage
        localStorage.setItem("teamData", JSON.stringify(teamData));
        
        // ✅ FIX 7: Save to Firebase
        if (hasDatabase()) {
        ...
        // ✅ FIX 9: Regenerate current schedule to remove deleted member
        const dateInput = document.getElementById("scheduleDate");
        if (dateInput && dateInput.value) {
            const currentDate = new Date(dateInput.value);
            if (typeof generateDailySchedule === 'function') {
                console.log(`🔄 Regenerating schedule without ${name}`);
                setTimeout(() => generateDailySchedule(currentDate), 500);
            }
        }
        
        showNotification(`${name} removed successfully from all systems`, "success");
        
        console.log(`✅ COMPLETE REMOVAL: ${name} (${role})`);
        console.log(`   - Removed from teamData.${category}`);
        console.log(`   - Removed from memberAvailabilitySettings`);
        console.log(`   - Removed from availabilityOverrides`);
        console.log(`   - Removed from leave calendar`);
...
// ==========================================
// ORPHANED MEMBERS CLEANUP FUNCTION
// ==========================================
...
function cleanupOrphanedMembers() {
    ...
    const currentSAs = (teamData && teamData.SAs) ? teamData.SAs : [];
    const currentApprentices = (teamData && teamData.apprentices) ? teamData.apprentices : [];
    
    console.log('Current SAs:', currentSAs);
    console.log('Current Apprentices:', currentApprentices);
    
    // Clean memberAvailabilitySettings.SAs
    if (memberAvailabilitySettings && memberAvailabilitySettings.SAs) {
        for (const name in memberAvailabilitySettings.SAs) {
            if (!currentSAs.includes(name)) {
                delete memberAvailabilitySettings.SAs[name];
                cleanedCount++;
                console.log(`   ✅ Removed orphaned SA: ${name}`);
            }
        }
    }
    
    // Clean memberAvailabilitySettings.apprentices
    if (memberAvailabilitySettings && memberAvailabilitySettings.apprentices) {
        for (const name in memberAvailabilitySettings.apprentices) {
            if (!currentApprentices.includes(name)) {
                delete memberAvailabilitySettings.apprentices[name];
                cleanedCount++;
                console.log(`   ✅ Removed orphaned Apprentice: ${name}`);
            }
        }
    }
    
    // Clean availabilityOverrides
    if (availabilityOverrides) {
        for (const key in availabilityOverrides) {
            const [name, role] = key.split('_');
            const isValid = (role === 'SA' && currentSAs.includes(name)) ||
                           (role === 'Apprentice' && currentApprentices.includes(name));
            
            if (!isValid) {
                delete availabilityOverrides[key];
                cleanedCount++;
                console.log(`   ✅ Removed orphaned availability: ${key}`);
            }
        }
    }
    
    // Clean monthlyLeaveSettings
    if (typeof monthlyLeaveSettings !== 'undefined' && monthlyLeaveSettings) {
        for (const monthKey in monthlyLeaveSettings) {
            if (monthlyLeaveSettings[monthKey]) {
                for (const memberKey in monthlyLeaveSettings[monthKey]) {
                    ...
                    if (!isValid) {
                        delete monthlyLeaveSettings[monthKey][memberKey];
                        cleanedCount++;
                        console.log(`   ✅ Removed orphaned leave: ${memberKey} from ${monthKey}`);
                    }
                }
            }
        }
    ...
    if (cleanedCount > 0) {
        ...
        if (typeof hasDatabase === 'function' && hasDatabase()) {
            if (typeof saveAvailabilityToFirebase === 'function') {
                saveAvailabilityToFirebase();
            }
            if (typeof saveLeaveSettingsToFirebase === 'function') {
                saveLeaveSettingsToFirebase();
        ...
        console.log(`✅ Cleanup complete: Removed ${cleanedCount} orphaned entries`);
        showNotification(`Cleaned up ${cleanedCount} orphaned entries`, 'success');
...
// Make cleanup function available in console for manual use
window.cleanupOrphanedMembers = cleanupOrphanedMembers;

console.log('%c🧹 Orphaned Members Cleanup Function Loaded', 'color: #f59e0b; font-weight: bold; font-size: 14px');
console.log('%c   Type cleanupOrphanedMembers() in console to run manual cleanup', 'color: #3b82f6; font-size: 12px');
