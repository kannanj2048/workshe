...
// Store shift overrides per date (to bypass automatic shift detection)
let shiftOverrides = JSON.parse(localStorage.getItem("shiftOverrides")) || {};
...
function generateScheduleForDate(date) {
    ...
    // Check for shift override for this specific date
    const shift = shiftOverrides[dateStr] || getCurrentShift(date);
...
function generateDailySchedule(dateSGT) {
    ...
    if (typeof hasDatabase === 'function' && hasDatabase()) {
        ...
        if (scheduleHistory[dateStr]) {
            displaySchedule(scheduleHistory[dateStr]);
        } else {
...
function openEditShiftModal() {
    ...
    if (form) {
        form.onsubmit = (e) => {
            ...
            if (schedule) {
                ...
                if (applyToDate) {
                    // Checkbox checked: Apply to this date only, remove any override
                    ...
                    // Checkbox unchecked: Override automatic shift detection for this date
                    shiftOverrides[dateStr] = newShift;
                    localStorage.setItem('shiftOverrides', JSON.stringify(shiftOverrides));
                    showNotification(`Shift override set to ${SHIFT_TYPES[newShift].name} for ${dateStr}! (Bypasses automatic detection)`, 'success');
                }
    ...
    if (clearBtn) {
        clearBtn.onclick = () => {
            // Clear the shift override for this specific date
            if (shiftOverrides[dateStr]) {
                delete shiftOverrides[dateStr];
                localStorage.setItem('shiftOverrides', JSON.stringify(shiftOverrides));
                
                // Regenerate schedule with automatic shift detection
                ...
                updateCurrentShiftInfo(currentDate);
                showNotification('Shift override cleared! Using automatic detection.', 'success');
            } else {
                showNotification('No override to clear for this date.', 'info');
            }
            modal.classList.remove('show');
        };
    }
