...
(function() {
    ...
    // 1. Clean memberAvailabilitySettings.SAs
    if (memberAvailabilitySettings && memberAvailabilitySettings.SAs) {
        const allSAs = Object.keys(memberAvailabilitySettings.SAs);
        allSAs.forEach(name => {
    ...
    // 2. Clean memberAvailabilitySettings.apprentices
    if (memberAvailabilitySettings && memberAvailabilitySettings.apprentices) {
        const allApprentices = Object.keys(memberAvailabilitySettings.apprentices);
        allApprentices.forEach(name => {
    ...
    // 3. Clean availabilityOverrides
    if (availabilityOverrides) {
        const allKeys = Object.keys(availabilityOverrides);
        allKeys.forEach(key => {
    ...
    if (typeof monthlyLeaveSettings !== 'undefined' && monthlyLeaveSettings) {
        for (const monthKey in monthlyLeaveSettings) {
            if (monthlyLeaveSettings[monthKey]) {
                const allMembers = Object.keys(monthlyLeaveSettings[monthKey]);
                allMembers.forEach(memberKey => {
                    ...
                    if (!isValid) {
                        ...
                        removedCount++;
                        console.log(`   ❌ Removed orphaned leave: ${memberKey}`);
                    }
                });
            }
        }
    ...
    console.log('='.repeat(60));
    console.log(`✅ CLEANUP COMPLETE! Removed ${removedCount} orphaned entries`);
    console.log('='.repeat(60));
